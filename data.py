...
def hred_article2ids(article_words, vocab):
  """Map the article words to their ids. Also return a list of OOVs in the article.
    ...
    for w in sent:
      ...
      if i == unk_id: # If w is OOV
        if w not in oovs: # Add to list of OOVs
          oovs.append(w)
        oov_num = oovs.index(w) # This is 0 for the first article OOV, 1 for the second article OOV...
        ids_tmp.append(vocab.size() + oov_num) # This is e.g. 50000 for the first article OOV, 50001 for the second...
      else:
        ids_tmp.append(i)
    ids.append(ids_tmp)
  return ids, oovs
...
def show_art_oovs(article, vocab, string_split):
  """Returns the article string, highlighting the OOVs by placing __underscores__ around them"""
  unk_token = vocab.word2id(UNKNOWN_TOKEN)
  if string_split == 'char':
    words = list(article)
  else:
    words = []
    for art in article:
      words.extend(art.split(' ' if string_split == 'space' else string_split))
    # words = article.split(' ' if string_split == 'space' else string_split)
  # words = article.split(' ')
...
def show_abs_oovs(abstract, vocab, article_oovs, string_split=None):
  ...
  if string_split == 'char':
    ...
    words = abstract.split(' ' if string_split == 'space' else string_split)
